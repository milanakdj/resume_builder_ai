...
if __name__ == "__main__":
    try:
        ...
        dataset_name = r"./dataset/master_resumes.jsonl"
        output_dataset_name = r"./dataset/dataset.csv"
        field_names = ["Input", "Output"]
        data_list = []
        i = 0

        with open(output_dataset_name, "r") as csvfile:
        ...
        while True:
            try:
                for json_str in modified_json_list[i:]:
                    ...
                    # complete_resume = "This is the completed resume"

                    print(f"Generated new response: {i+1} for the Dataset")
                ...
                # Find the dictionary part (after "429 RESOURCE_EXHAUSTED. ")
                dict_str = error_str.split("429 RESOURCE_EXHAUSTED. ", 1)[1]
        ...
        print(f"Saved changes to the new dataset. New total length: {len(data_list)}")

        print("Total time Ran for: ", str(time.time() - start_time))