...
def main(job_description = ""):
    ...
    file_name = "enhanced_resume_"+ str(datetime.now().strftime("%d-%m-%Y_%H-%M-%S"))+".docx"
...
def generate_resume(job_description):
    final_path = main(job_description= job_description)
    return "Hello ujjwal your new Resume has been created in " + final_path


demo = gr.Interface(fn=generate_resume, inputs="textbox", outputs="textbox", title = "Resume generator")
